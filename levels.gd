...
# id-uri pentru elementele din tileset
const ID_IARBA = 3
...
func generate_fixed_level(lungime: int, s_id: int):
	# generam un nivel standard folosind un seed anume
	seed(s_id)
	clear()
...
func generate_concatenated_level():
	# lipim doua bucati de nivel diferite ca sa fie mai lung
	clear()
	seed(SEED_NIVEL_1)
...
func _build_terrain(lungime: int, offset_x: int = 0):
	...
	while x < lungime:
		...
		# logica pentru balti de apa (nu apar in zonele de start/final)
		...
		# punem stratul de suprafata si pamantul de dedesubt
...
func _build_platforms_and_items(lungime: int, offset_x: int = 0):
	...
	# bucla pentru generat platforme plutitoare si obiecte pe ele
	...
	# plasam usa de final si indicatoarele
	var usa_x = offset_x + lungime - 20 
	...
	# salvam pozitia globala a usii ca sa stie jucatorul unde sa termine nivelul
	usa_world_pos = to_global(map_to_local(Vector2i(usa_x, NIVEL_PODEA - 1)))

func _spawn_box(tile_x: int, tile_y: int):
	# instantiem o cutie si o punem corect in coordonate de pixeli
